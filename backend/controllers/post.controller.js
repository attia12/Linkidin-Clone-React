...
export const deletePost = async (req,res)=>{
    try {
        ...
        //check the user is thee author of the post
        if(post.author.toString()!==userId.toString()) {
            return res.status(403).json({ message: "you are Authorized to delete this post" });   
        }
        if(post.image) {
...
export const likePost = async (req, res) => {
    try {
        ...
        if(post.likes.includes(userId)) {
            ...
            // create a notification if the post owner is not the like owner
            if(post.author.toString()!==userId.toString()) {
                const newNotification = new Notification({
                    recipient: post.author,
                    type: "like",
